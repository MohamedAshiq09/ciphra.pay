...
// Configuration
const RPC_URL =
  "https://starknet-sepolia.g.alchemy.com/v2/TI9jXk5bEpPAnw6QwoziO";
const DEPLOYER_ADDRESS =
  "0x04bAEAE1872c93c283C9E660364FAb37B22A5BA5276d176DAF363d5B1d91E78c";
...
async function main() {
  ...
  if (!PRIVATE_KEY) {
    ...
    console.log(
      "Please run with: DEPLOYER_PRIVATE_KEY=your_key npx ts-node scripts/deploy_atomic_swap_only.ts"
    );
    process.exit(1);
  }
  ...
  const contractPath = path.join(
    __dirname,
    "../target/dev/ciphra_pay_AtomicSwap.contract_class.json"
  );
  const compiledContractPath = path.join(
    __dirname,
    "../target/dev/ciphra_pay_AtomicSwap.compiled_contract_class.json"
  );
  ...
  if (fs.existsSync(compiledContractPath)) {
    compiledContract = JSON.parse(
      fs.readFileSync(compiledContractPath, "utf-8")
    );
    console.log("✅ Found compiled contract class (CASM)");
  } else {
    console.log(
      "⚠️  No compiled contract class found - declare might fail on some networks"
    );
  }
  ...
  try {
    ...
    const atomicSwapIndex = deployments.contracts.findIndex(
      (c: any) => c.contractName === "AtomicSwap"
    );
    const newEntry = {
    ...
    console.log("\nNext steps:");
    console.log(
      "1. Update backend .env with new STARKNET_SWAP_CONTRACT_ADDRESS"
    );
    ...
    if (error.message?.includes("already declared")) {
      console.log(
        "⚠️  Contract already declared, deploying with existing class hash..."
      );
      ...
      const existingClassHash = existingDeployments.contracts.find(
        (c: any) => c.contractName === "AtomicSwap"
      )?.classHash;
