...
class StarknetDeployer {
  ...
  constructor() {
    ...
    if (!privateKey || !accountAddress) {
      throw new Error(
        "Please set DEPLOYER_PRIVATE_KEY and DEPLOYER_ADDRESS in .env"
      );
    }
  ...
  private readContract(contractName: string) {
    ...
    const sierraPath = path.join(
      __dirname,
      `../target/dev/${base}.contract_class.json`
    );
    const casmPath = path.join(
      __dirname,
      `../target/dev/${base}.compiled_contract_class.json`
    );
  ...
  async deployAll() {
    ...
    const bridgeAddr = await this.deployContract(
      "BridgeConnector",
      bridgeHash,
      [this.account.address]
    );
