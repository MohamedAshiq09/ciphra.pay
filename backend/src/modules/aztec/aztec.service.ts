...
export class AztecService implements OnModuleInit {
  ...
  private async initialize() {
    try {
      this.logger.log(
        `Connecting to Aztec PXE at ${this.config.aztecPxeUrl}...`,
      );
      ...
      this.logger.log(
        `✅ Loaded PrivateAtomicSwap contract at ${this.contractAddress.toString()}`,
      );
      ...
      if (this.contract) {
        ...
        this.logger.warn(
          `⚠️  Contract artifact not loaded - skipping verification`,
        );
      }
    } catch (error) {
  ...
  async getCompletedSwaps(): Promise<bigint> {
    try {
      const completed = await this.contract.methods
        .get_completed_swaps()
        .simulate();
      return completed;
    } catch (error) {
  ...
  async getSwapStatus(swapId: string): Promise<SwapStatus> {
    try {
      ...
      const status = await this.contract.methods
        .get_swap_status(swapIdField)
        .simulate();
      return Number(status) as SwapStatus;
    } catch (error) {
      this.logger.error(
        `Failed to get swap status for ${swapId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getRevealedSecret(swapId: string): Promise<string | null> {
    try {
      const swapIdField = Fr.fromString(swapId);
      const secret = await this.contract.methods
        .get_revealed_secret(swapIdField)
        .simulate();
      ...
      this.logger.error(
        `Failed to get revealed secret for ${swapId}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async getTargetChain(swapId: string): Promise<TargetChain> {
    try {
      const swapIdField = Fr.fromString(swapId);
      const chain = await this.contract.methods
        .get_target_chain(swapIdField)
        .simulate();
      return Number(chain) as TargetChain;
    } catch (error) {
      this.logger.error(
        `Failed to get target chain for ${swapId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getTargetSwapId(swapId: string): Promise<string> {
    try {
      const swapIdField = Fr.fromString(swapId);
      const targetId = await this.contract.methods
        .get_target_swap_id(swapIdField)
        .simulate();
      return targetId.toString();
    } catch (error) {
      this.logger.error(
        `Failed to get target swap ID for ${swapId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getTokenAddress(swapId: string): Promise<string> {
    try {
      const swapIdField = Fr.fromString(swapId);
      const tokenAddr = await this.contract.methods
        .get_token_address(swapIdField)
        .simulate();
      return tokenAddr.toString();
    } catch (error) {
      this.logger.error(
        `Failed to get token address for ${swapId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getSwapDetails(swapId: string): Promise<AztecSwapDetails> {
    try {
      const [status, targetChain, targetSwapId, tokenAddress, secret] =
        await Promise.all([
  ...
  /**
   * Initiate a new swap on Aztec (as counterparty)
   *
   * Called by backend to create counterparty swap when source chain swap is detected
   *
   * @param dto - Initiate swap parameters
   * @returns Transaction hash
   */
  async initiateSwap(dto: InitiateSwapDto): Promise<string> {
    try {
      this.logger.log(`🔵 Initiating Aztec swap ${dto.swapId}...`);
      ...
      this.logger.log(`   Time lock: ${dto.timeLockDuration}s`);
      this.logger.log(
        `   Target: ${TargetChain[dto.targetChain]} (${dto.targetSwapId})`,
      );

      // TODO: Implement actual contract call when wallet is configured
      // const swapIdField = Fr.fromString(dto.swapId);
      // const recipientAddr = AztecAddress.fromString(dto.recipient);
      // const tokenAddr = AztecAddress.fromString(dto.tokenAddress);
      // const amountField = Fr.fromString(dto.amount);
      // const hashLockField = Fr.fromString(dto.hashLock);
      //
      ...
      //     hashLockField,
      //     dto.hashType,
      //     dto.timeLockDuration,
      //     dto.targetChain,
      //     Fr.fromString(dto.targetSwapId),
      //   )
      //   .send()
      //   .wait();
      //
      ...
      this.logger.warn(
        '⚠️ Initiate swap not yet implemented - needs wallet integration',
      );
      this.logger.log(
        '📋 Would create swap with params:',
        JSON.stringify(dto, null, 2),
      );

      // Return mock tx hash for now
      return `aztec_mock_tx_${Date.now()}`;
    } catch (error) {
  ...
  async getFeeRecipient(): Promise<string> {
    try {
      const recipient = await this.contract.methods
        .get_fee_recipient()
        .simulate();
  ...
  async getCollectedFees(tokenAddress: string): Promise<bigint> {
    try {
      ...
      const fees = await this.contract.methods
        .get_collected_fees(tokenAddr)
        .simulate();
      return fees;
    } catch (error) {
  ...
  async completeSwap(dto: CompleteSwapDto): Promise<string> {
    try {
      ...
      this.logger.warn(
        'Complete swap not yet implemented - needs wallet integration',
      );
      return 'pending';
    } catch (error) {
  ...
  async refundSwap(dto: RefundSwapDto): Promise<string> {
    try {
      ...
      this.logger.warn(
        'Refund swap not yet implemented - needs wallet integration',
      );
      return 'pending';
    } catch (error) {
